...
export default function Financeiro() {
  ...
  const ultimosPagamentos = useMemo(() => mensalidades.filter((m) => m.status === "PAGO").sort((a, b) => dataLocal(b.pagamento) - dataLocal(a.pagamento)), [mensalidades]);
  const proximosVencimentos = useMemo(() => mensalidades.filter((m) => m.status !== "PAGO" && dataLocal(m.vencimento) >= inicioDoMes(new Date())).sort((a, b) => dataLocal(a.vencimento) - dataLocal(b.vencimento)), [mensalidades]);
  ...
  return <div className="finance-page"><Navbar /><Sidebar /><main className="finance-main">
    ...
    <section className="f-grid">
      ...
      <Table title="♙　Últimos Pagamentos" rows={ultimosPagamentos} paid />
      <Table title="▣　Próximos Vencimentos" rows={proximosVencimentos} />
    </section>
  </main></div>;
}
